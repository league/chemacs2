...
- =nix-elisp-bundle= A directory containing elisp dependencies bundled by the
  nix package manager. The derivation can be specified with something like
  =(emacs.withPackages (ep: [ ep.avy-zap … ])).deps=, but to avoid it being
  garbage collected, =.emacs-profiles.el= must also be kept by nix; see
  [[https://github.com/league/chemacs2nix][league/chemacs2nix]] for a home-manager module to do that.
